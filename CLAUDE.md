...
Firestore rules e índices compuestos están en `firestore.rules` y `firestore.indexes.json`, y **se despliegan solos**: `.github/workflows/firebase-hosting-merge.yml` corre `firebase deploy --only firestore:rules,firestore:indexes` en cada push a `main`, antes del hosting. No hay que pegar nada a mano en la consola, y lo pegado a mano se sobrescribe en el siguiente merge.

**`firestore.indexes.json` tiene que ser un superconjunto de lo que hay en el proyecto.** Si al proyecto le sobra un índice, el CLI pregunta si borrarlo, y preguntar en modo no interactivo aborta el despliegue entero — incluido el hosting. Si eso pasa, **no añadas `--force`**: borraría esos índices de verdad. Reconcilia con `npx firebase-tools@13 firestore:indexes --project vivai-now` y añade al archivo lo que falte. Si añades una consulta con filtro + orden, añade su índice ahí.
...
- **Atribución al dueño**: `PaymentService.create`, `ExpenseService.create`, `UtilityServiceService.create` y `ServiceAssignmentService.save` resuelven `ownerId` desde `properties/{id}.ownerId`, no desde el uid de quien ejecuta la acción — si no, lo creado por un colaborador queda invisible para el dueño. **Aplica también a `memberUids`**, que se deriva de ese dueño: un documento fuera del círculo no solo no se ve, sino que puede denegar el listado completo de su colección.
