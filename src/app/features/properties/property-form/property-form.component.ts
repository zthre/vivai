    ...
    <div>
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <div class="bg-white rounded-xl border border-warm-200 shadow-sm p-6 space-y-5 h-fit">
            <p class="text-xs font-semibold text-warm-500 uppercase tracking-wide">Información básica</p>
          ...
          <!-- Col 2: Ocupación -->
          <div class="bg-white rounded-xl border border-warm-200 shadow-sm p-6 space-y-4 h-fit">
            ...
            @if (form.get('isOccupied')?.value) {
              <div class="space-y-4 p-4 bg-warm-50 rounded-lg border border-warm-200">
                ...
                <div>
                  <label class="block text-sm font-medium text-warm-700 mb-1.5">Cantidad de personas</label>
                  <input formControlName="residentCount" type="number" min="1" placeholder="Ej: 2"
                    class="w-full px-3 py-2.5 border border-warm-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
        ...
        <!-- Actions (full width) -->
        <div class="flex gap-3 max-w-sm ml-auto">
