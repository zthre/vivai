    ...
    <div class="max-w-lg">
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="bg-white rounded-xl border border-warm-200 shadow-sm p-6 space-y-5">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <!-- Precio de renta (no aplica a venta) -->
          @if (form.get('status')?.value !== 'disponible_venta') {
            <div>
        ...
        <!-- Precio de venta (solo si disponible_venta) -->
        @if (form.get('status')?.value === 'disponible_venta') {
          <div>
...
export class UnitFormComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    rentPrice: [null as number | null, Validators.min(1)],
    salePrice: [null as number | null, Validators.min(1)],
